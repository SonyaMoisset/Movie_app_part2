                ...
                public void onClick(View v) {
                    ...
                    if (position != RecyclerView.NO_POSITION) {
                        ...
                        Intent webIntent = new Intent(Intent.ACTION_VIEW,
                                Uri.parse(context.getString(R.string.youtube_web_intent) + trailerId));
                        Intent appIntent = new Intent(Intent.ACTION_VIEW,
                                Uri.parse(context.getString(R.string.youtube_app_intent) + trailerId));

                        try {
                            appIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                            appIntent.putExtra(context.getString(R.string.TRAILER_ID_KEY), trailerId);

                            context.startActivity(appIntent);
                        } catch (ActivityNotFoundException ex) {
                            webIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                            webIntent.putExtra(context.getString(R.string.TRAILER_ID_KEY), trailerId);
